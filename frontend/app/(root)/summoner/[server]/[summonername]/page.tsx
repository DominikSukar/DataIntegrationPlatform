...
const SummonerPage = async ({ params }: PageProps) => {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex flex-col items-center">
        {matches.length ? (
          matches.map((match, index) => <Game key={index} match={match} />)
        ) : (
          <div className="p-2 bg-white bg-opacity-20 backdrop-blur-md border-2 border-white border-opacity-30 rounded-[10px] overflow-hidden">
            No ranked 5x5 matches found
          </div>
        )}
      </div>
    </div>
